...
def readHRFile():
    file_stream = io.open("/home/ashleyjiang28/Desktop/excel/IT staff list HR_staff.csv", mode='r', buffering=-1, encoding=None,
...
lname_HR = header_HR.index("last")
fname_HR = header_HR.index("first")
location_HR = header_HR.index("prog")
desc_HR = header_HR.index("working title")
        ...
        if lname == row[lname_HR]:
            try:
                ...
                pass
                        ...
                        # of the for loop
        with open("output_not_match.csv","ab") as f:
            writer=csv.writer(f)
            writer.writerow(row) #which row is being written here?
